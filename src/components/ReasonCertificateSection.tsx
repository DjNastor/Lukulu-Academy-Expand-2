...
export function ReasonCertificateSection() {
 return <section id="reason-certificate" className="certificate-section signal-section section-pad section-paper" aria-labelledby="reason-certificate-heading"><div className="page-shell">
  <SectionIntro headingId="reason-certificate-heading" number="03" eyebrow="Flagship remote programme" title="Create, mix and release in Reason 14" description="A practical 12-week course for new and developing producers. Make one complete track in Afro House, Amapiano, Hip-Hop, Pop or Electronic music." />
  <Reveal className="certificate-hero"><div className="certificate-hero__copy">
   <span className="certificate-badge"><AwardIcon aria-hidden="true" /> Lukulu certificate of completion</span><h3>Learn by finishing one real project</h3><p>Each week adds a new skill to the same song. You will plan, produce, record, mix, master and prepare it for release.</p>
   ...
   <div className="certificate-hero__actions"><Link className="button button-primary" to={enquiry}>Ask about the next intake <ArrowRightIcon/></Link><Link className="button button-quiet" to="/student/demo">Preview the student portal <BookOpenCheckIcon/></Link></div><p className="certificate-price-note">Course price and live-session dates are confirmed during enrolment.</p>
  </div><div className="certificate-hero__signal"><Music2Icon/><p className="console-label">YOUR FINAL RESULT</p><strong>One finished, release-ready track</strong><span>Plus the project files and documents needed to present and release your work.</span></div></Reveal>
  ...
  <div className="certificate-outline"><div className="certificate-outline__head"><div><p className="console-label">FULL COURSE OUTLINE</p><h3>What you learn each week</h3></div><p>Open any week to read the short lesson summary.</p></div><div className="certificate-weeks">{weeks.map((x,i)=><details key={x[0]} open={i===0}><summary><span>{x[0]}</span><strong>{x[1]}</strong><span aria-hidden="true">+</span></summary><p>{x[2]}</p></details>)}</div></div>
  <div className="certificate-results"><section><p className="console-label">WHAT YOU MAKE</p><h3>Your final course package</h3><ul>{outputs.map(x=><li key={x}><CheckCircle2Icon/> {x}</li>)}</ul></section><section><p className="console-label">HOW YOU ARE MARKED</p><h3>Simple, clear assessment</h3><dl>{marks.map(x=><div key={x[0]}><dt>{x[0]}</dt><dd>{x[1]}</dd></div>)}</dl><p>To complete the course, submit every major project, finish at least 80% of the learning activities and earn at least 50% overall.</p></section></div>
  <div className="certificate-footer"><HeadphonesIcon/><div><strong>Designed for remote students</strong><p>Lessons use Reason’s built-in tools. Videos are short, workshops are recorded and tasks can be completed with headphones.</p></div><Link to={enquiry}>Enquire now <ArrowRightIcon/></Link></div>
 </div></section>;
}
